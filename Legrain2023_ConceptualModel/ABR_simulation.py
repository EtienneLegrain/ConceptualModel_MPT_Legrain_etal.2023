...
StartPosition = (0.2, 0.6, 0.7, 0.7, -0.1, 8, 20, -0.4, 11.8, 98, 4, 23, 1219, 61)
...
def Phi(i,vt,parameters):
    ...
    if i/2>(2000-parameters[12]):
        ...
        if S[0]=="g":
            if test_threshold_gd>parameters[13] and test_threshold_dg>parameters[10]:
                S[0] = "d"
                S[1] = i/2*0.1
        else :
            if test_threshold_dg<parameters[10] and test_threshold_gd<parameters[13]:
                S[0] = "g"
...
vOiMPT = StartPosition[13]
S = ["g",0]

param = [aEsii,aEcoi,aOi,agi,adi,taui,kEsii,kEcoi,kOi,v0i,v1i,vii,ageMPT,vOiMPT]
...
#####################################################################
#Extraction of the best parameters list to copy and paste in the ABR_simulation_plot.py.py program

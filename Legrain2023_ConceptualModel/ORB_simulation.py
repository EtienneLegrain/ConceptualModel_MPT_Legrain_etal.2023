...
StartPosition = (1, 0.1, 0.6, 0.6, -0.1, 8.5, 23, -0.5, 12, 83, 7, 52)
...
#####################################################################
#Extraction of the best parameters list to copy and paste in the ORB_simulation_plot.py program

#Recovering of parameters from which we obtain the minimum residuals
maxindex = np.argmax(steps.get_log_prob(flat=True))
allresiduals = steps.get_log_prob(flat=True)
meilleurproba = np.max(steps.get_log_prob(flat=True))
allparams = steps.get_chain(flat=True)

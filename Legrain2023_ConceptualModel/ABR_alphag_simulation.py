...
StartPosition = (0.3, 0.7, 0.5, 0.8, -0.1, 5, 38, -0.7, 15, 108, 4, 23, 1168, 0.4)
...
def Phi(i,vt,parameters):
    ...
    if S[0]=="g" :
        if i/2>(2000-parameters[12]):
            ...
            dvdt = -parameters[0]*Esitr[i]-parameters[1]*Ecotr[i]-parameters[2]*EnOtr[i]+parameters[13]
    else :
...
agiMPT = StartPosition[13]
S = ["g",0]

param = [aEsii,aEcoi,aOi,agi,adi,taui,kEsii,kEcoi,kOi,v0i,v1i,vii,ageMPT,agiMPT]
...
#####################################################################
#Extraction of the best parameters list to copy and paste in the ABR_alphag_simulation_plot.py program
